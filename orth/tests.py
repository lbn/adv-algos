import numpy as np
from orth import Tree, Orth, OrthTree
...
def test_succ(IOrth):
    ...
    assert succ_err == None, "actual: {}".format(succ_err)
...
def test_pred(IOrth):
    ...
    for i in range(1,64):
        ...
        assert predecessor < successor, "{} vs {} [i={}]".format(previous,successor,successor_i)
    pred_err = IOrth.predecessor(nums,min(nums)-1)
    assert pred_err == None, "actual: {}".format(pred_err)
